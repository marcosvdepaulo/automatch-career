...
class NotionDB:
    ...
    def testar_conexao(self):
        ...
        if not self.token or not self.database_id:
            print("❌ NOTION_TOKEN ou NOTION_DATABASE_ID não foram carregados do ambiente.")
            print("   Verifique se o .env existe na raiz do projeto e se load_dotenv() foi chamado.")
            return False
        ...
        try:
            ...
            if response.status_code == 401:
                print("❌ Notion retornou 401 (não autorizado) — o NOTION_TOKEN está errado ou expirado.")
            elif response.status_code == 404:
                print("❌ Notion retornou 404 — o NOTION_DATABASE_ID está errado, ou o database")
                print("   ainda não foi compartilhado com a integration (aba Share, no Notion).")
            else:
                print(f"❌ Notion retornou {response.status_code}: {response.text[:200]}")
            return False
