...
GitHub Actions (segunda, 9h) → `main.py` → constrói um candidato explícito → testa conexão Notion → coleta vagas → cria `OpportunityProfile` → `matcher.assess(candidate, opportunity)` → filtro de corte 40% → persistência e Notion.
...
6. **Histórico:** a escala do matcher legado não era bem calibrada. A arquitetura de domínio substituiu os pesos globais; recalibração continua fora do escopo desta etapa.
...
4. **Robustez do matcher** — evoluir a detecção além da correspondência lexical usando aliases da ontologia, sem mudar as fronteiras de domínio.
